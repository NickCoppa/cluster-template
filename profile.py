...
for i in range(6):
  ...
  node.addService(pg.Execute(shell="sh", command="sudo -H -u nc835571 bash -c '/local/repository/ssh_setup.sh'"))
 
  node.addService(pg.Execute(shell="sh", command="sudo su nc835571 -c 'cp /local/repository/source/* /users/nc835571'"))
